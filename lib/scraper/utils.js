...
export async function extractPrice(...elements) {
  for (const element of elements) {
    const priceText = await element.text().trim();
...
export async function extractCurrency(element) {
  const currencyText = await element.text().trim().slice(0, 1);
